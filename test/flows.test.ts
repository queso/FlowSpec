...
describe('YAML Flow Fixtures', () => {
  it('should have all required flow fixture files', () => {
    ...
    expect(existsSync(join(VALID_FLOWS_DIR, 'form-submission.flow.yaml'))).toBe(true);
    expect(existsSync(join(VALID_FLOWS_DIR, 'navigation.flow.yaml'))).toBe(true);
    expect(existsSync(join(INVALID_FLOWS_DIR, 'missing-name.flow.yaml'))).toBe(true);
    expect(existsSync(join(INVALID_FLOWS_DIR, 'invalid-step.flow.yaml'))).toBe(true);
    expect(existsSync(join(INVALID_FLOWS_DIR, 'empty-steps.flow.yaml'))).toBe(true);
  });

  it('valid flows should parse as valid YAML and pass schema', () => {
    const files = ['login.flow.yaml', 'form-submission.flow.yaml', 'navigation.flow.yaml'];
  ...
  it('invalid flows should fail schema validation', () => {
    const files = ['missing-name.flow.yaml', 'invalid-step.flow.yaml', 'empty-steps.flow.yaml'];
