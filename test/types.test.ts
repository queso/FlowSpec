...
describe('FlowSpec Types', () => {
  ...
  it('StepAction should validate visit, click, fill, and select', () => {
    ...
    expect(StepActionSchema.safeParse({ fill: { Email: 'test@example.com' } }).success).toBe(true);
    expect(StepActionSchema.safeParse({ select: { Country: 'US' } }).success).toBe(true);
    expect(StepActionSchema.safeParse({}).success).toBe(false);
  });

  it('StepAssertion should validate url, visible, matches, and not_visible', () => {
    expect(StepAssertionSchema.safeParse({ url: '/dashboard' }).success).toBe(true);
    expect(StepAssertionSchema.safeParse({ visible: 'Welcome' }).success).toBe(true);
    expect(StepAssertionSchema.safeParse({ matches: 'Order #\\d+' }).success).toBe(true);
    expect(StepAssertionSchema.safeParse({ not_visible: 'Error' }).success).toBe(true);
  ...
  it('FlowSpec should validate complete flow with required fields', () => {
    ...
    expect(FlowSpecSchema.safeParse({ name: 'test', description: 'test', steps: [] }).success).toBe(false);
  });
  ...
  it('FlowError should validate error with message', () => {
    const error = { message: 'Element not found', step: 2, action: { click: 'Submit' } };
